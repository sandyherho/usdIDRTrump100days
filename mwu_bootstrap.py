...
"""
Mann-Whitney U Test Analysis Script with Bootstrap for USD/IDR Exchange Rate
---------------------------------------------------------------------------
...
This script performs Mann-Whitney U tests with bootstrap resampling between
...
def bootstrap_mannwhitneyu(x, y, n_bootstrap=10000):
    """
    Perform Mann-Whitney U test with bootstrap resampling.
    ...
    for i in range(n_bootstrap):
        ...
        # Calculate Mann-Whitney U test for this bootstrap sample
        try:
            u_stat, p_value = stats.mannwhitneyu(x_sample, y_sample, alternative='two-sided')
    ...
    return u_mean, p_mean, p_median, p_ci, reject_ratio, bootstrap_data
...
def analyze_inauguration_impact(pre_data, post_data, entire_data, n_bootstrap=10000, 
    ...
    """
    Analyze the impact of inauguration on USD_IDR using Mann-Whitney U test with bootstrap resampling
    ...
    try:
        ...
        # Direct Mann-Whitney U test without bootstrap
        u_stat, p_value = stats.mannwhitneyu(pre_values, post_values, alternative='two-sided')
        
        # Bootstrapped Mann-Whitney U test
        u_mean, p_mean, p_median, p_ci, reject_ratio, bootstrap_data = bootstrap_mannwhitneyu(
            pre_values, post_values, n_bootstrap=n_bootstrap
        )
        
        # Save bootstrap data for visualization
        if not bootstrap_data.empty:
            bootstrap_filename = "mwu_bootstrap_pre_vs_post_usd_idr.csv"
        ...
        # Direct Mann-Whitney U test without bootstrap
        u_stat, p_value = stats.mannwhitneyu(pre_values, entire_values, alternative='two-sided')
        ...
        # Save bootstrap data for visualization
        if not bootstrap_data.empty:
            bootstrap_filename = "mwu_bootstrap_pre_vs_entire_usd_idr.csv"
            bootstrap_filepath = os.path.join(output_dir, bootstrap_filename)
        ...
        # Direct Mann-Whitney U test without bootstrap
        u_stat, p_value = stats.mannwhitneyu(post_values, entire_values, alternative='two-sided')
        ...
        # Save bootstrap data for visualization
        if not bootstrap_data.empty:
            bootstrap_filename = "mwu_bootstrap_post_vs_entire_usd_idr.csv"
            bootstrap_filepath = os.path.join(output_dir, bootstrap_filename)
        ...
        print(f"Error in bootstrapped Mann-Whitney U analysis: {e}")
        traceback.print_exc()
...
def generate_report(results, inauguration_date):
    """
    Generate a comprehensive report based on the Mann-Whitney U test results
    ...
    report_content = f"""# Bootstrapped Mann-Whitney U Test Analysis of USD/IDR Exchange Rate
## Analysis of USD/IDR Exchange Rate Before and After Presidential Inauguration (January 20, 2025)

This report presents the results of Mann-Whitney U tests comparing USD/IDR exchange rates before and after the presidential inauguration on {inauguration_date.strftime('%B %d, %Y')}. The analysis includes bootstrap resampling for robust confidence intervals.
...
"""
    ...
    if not results_df.empty:
        ...
        if pre_post_row['Robust significance']:
            report_content += f"- **Robust statistical significance** - The difference between pre and post-inauguration periods is statistically significant (rejection ratio: {pre_post_row['Rejection ratio']:.2f}, p-value: {pre_post_row['Bootstrap p-value (mean)']:.4f}, 95% CI: [{pre_post_row['p-value CI lower']:.4f}, {pre_post_row['p-value CI upper']:.4f}])\n"
        elif pre_post_row['Rejection ratio'] > 0.5:
            report_content += f"- **Moderate evidence** - The difference between pre and post-inauguration periods shows moderate evidence of statistical significance (rejection ratio: {pre_post_row['Rejection ratio']:.2f}, p-value: {pre_post_row['Bootstrap p-value (mean)']:.4f}, 95% CI: [{pre_post_row['p-value CI lower']:.4f}, {pre_post_row['p-value CI upper']:.4f}])\n"
        else:
            report_content += f"- **No robust evidence** - The difference between pre and post-inauguration periods does not show robust evidence of statistical significance (rejection ratio: {pre_post_row['Rejection ratio']:.2f}, p-value: {pre_post_row['Bootstrap p-value (mean)']:.4f}, 95% CI: [{pre_post_row['p-value CI lower']:.4f}, {pre_post_row['p-value CI upper']:.4f}])\n"
...
def save_report(report_content, output_dir="results/stats", filename="bootstrapped_mannwhitneyu_analysis_usd_idr.txt"):
    """
...
def main():
    """
    Main function to run the bootstrapped Mann-Whitney U test analysis
    ...
    try:
        ...
        # Perform Mann-Whitney U tests with bootstrap
        print(f"\nPerforming Mann-Whitney U tests with bootstrap resampling ({n_bootstrap} iterations) for USD/IDR exchange rate:")
        ...
        # Save results to CSV
        results_df = pd.DataFrame(results)
        results_csv = os.path.join(output_dir, "bootstrapped_mannwhitneyu_results_usd_idr.csv")
        results_df.to_csv(results_csv, index=False)
        print(f"Bootstrapped Mann-Whitney U test results saved to {results_csv}")
        ...
        # Save report
        save_report(report_content, report_dir, "bootstrapped_mannwhitneyu_analysis_usd_idr.txt")
        
        print("\nBootstrapped Mann-Whitney U test analysis for USD/IDR complete.")
